...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        #get values then apply nn functions then put values into given variables
        weight_embed = nn.Parameter(torch.empty(self.embed_size * n_features, hidden_size))
        bias_embed = nn.Parameter(torch.empty(hidden_size))
        weight_logit = nn.Parameter(torch.empty(hidden_size, n_classes))
        bias_logit = nn.Parameter(torch.empty(n_classes))
        
        nn.init.xavier_uniform_(weight_embed)
        nn.init.uniform_(bias_embed)
        nn.init.xavier_uniform_(weight_logit)
        nn.init.uniform_(bias_logit)
        
        self.embed_to_hidden_weight = weight_embed
        self.embed_to_hidden_bias = bias_embed
        self.hidden_to_logits_weight = weight_logit
        self.hidden_to_logits_bias = bias_logit
    ...
    def forward(self, w):
        ...
        # Lookup embeddings for input words and compute logits
        embed_look = self.embedding_lookup(w)
        logits = torch.matmul(F.relu(torch.matmul(embed_look, self.embed_to_hidden_weight) + self.embed_to_hidden_bias), self.hidden_to_logits_weight) + self.hidden_to_logits_bias
