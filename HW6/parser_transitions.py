...
class PartialParse(object):
    def __init__(self, sentence):
        ...
        # Assign initial stack, buffer, and dependencies to class attributes
        self.dependencies = []
        self.stack = ["ROOT"]
    ...
    def parse_step(self, transition):
        ...
        if transition == "S":
            ...
            depen = self.stack.pop(-2)
            top = self.stack[-1]
            self.dependencies.append((top, depen))
        elif transition == "RA":
            depen = self.stack.pop(-1)
            top = self.stack[-1]
            self.dependencies.append((top, depen))
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        for par, shift in zip(unfinished_parses, model.predict(unfinished_parses)):
            par.parse_step(shift)
        unfinished_parses = [par for par in unfinished_parses if len(par.buffer) > 0 or len(par.stack) > 1]

        dependencies = []
        for part_parse in partial_parses:
            if part_parse not in unfinished_parses:
                if part_parse.dependencies:
                    dependencies.append(part_parse.dependencies)
                else:
                    dependencies.append([])
    
        if not unfinished_parses:
            break
